...
public class VisualAtualizarCadastroFuncionario extends javax.swing.JDialog {
    ControleFuncionario controleFuncionario = new ControleFuncionario();
    FuncionarioLogado funcionarioLogado = new FuncionarioLogado();
    Funcionario func = new Funcionario();
    Endereco end = new Endereco();

    VisualAtualizarCadastroFuncionario() {
        setModal(true);
        initComponents();
    ...
    private void initComponents() {
        ...
        setTitle("Atualizar cadastro de funcionário");

        jLabel1.setFont(new java.awt.Font("Times New Roman", 0, 24)); // NOI18N
        jLabel1.setText("Atualizar cadastro de Funcionário");
        ...
        jLabel16.setText("*Preencha todos os campos.");
                ...
                if(verificarEmail){
                    if(verificarDataNascimento){
                        if(ValidarCPF.verificar(jTFCpf.getText())){
                            ...
                            String resultadoatualizacao = controleFuncionario.atualizarCadastroFuncionario(
                                    funcionarioLogado, func);
                            if(resultadoatualizacao.equals("")){
                                funcionarioLogado.setEmail(func.getEmail());
                                JOptionPane.showMessageDialog(null, "Atualização de cadastro de funcionário(a) "+
                                        jTFNomeCompleto.getText()+" realizado com sucesso");
                                limparCampos();
    ...
    public boolean validarCamposTextos(){
        if(jTFBairro.getText().length()<=0 || jTFCargo.getText().length()<=0 || jTFNResidencial.getText().length()<=0 || 
                ...
                || jTFRua.getText().length()<=0 || jTFCep.getText().length()<=0 ||
                jTFEmail.getText().length()<=0 || jTFComplemento.getText().length()<=0 || 
    ...
    public void preencherCamposFuncionario(Funcionario funcionario){
        String dataNascimento = ValidarData.
                converterDataParaFormatoDaMascara(funcionario.getData_nascimento());
        
        end.setId(funcionario.getEndereco().getId());
        ...
        jTFCidade.setText(funcionario.getEndereco().getCidade());
        jTFEstado.setText(funcionario.getEndereco().getEstado());
        jTFNResidencial.setText(funcionario.getEndereco().getNumero());
        jTFComplemento.setText(funcionario.getEndereco().getComplemento());
        
        func.setId(funcionario.getId());
        jFTFDNascimento.setText(dataNascimento);
        jTFCargo.setText(funcionario.getCargo());
        jTFCpf.setText(Long.toString(funcionario.getCpf()));
        jTFEmail.setText(funcionario.getEmail());
        jTFNomeCompleto.setText(funcionario.getNome());
        jTFRg.setText(Long.toString(funcionario.getRg()));
